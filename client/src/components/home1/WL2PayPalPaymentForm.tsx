...
function WL2PayPalProvider({ orderId, paymentId, onComplete, onError }: {
  orderId: string | null; paymentId: number | null;
  ...
  const captureOrder = trpc.paypal.captureWl2OneTimeOrder.useMutation({
    onSuccess: (data) => { if (resolvedPaymentId !== null) onComplete(resolvedPaymentId, data.captureId); },
    onError: (error) => { onError(`Payment confirmation failed: ${error.message}`); setSubmitting(false); },
  });
  if (!orderId || paymentId === null) {
    return <div className="flex flex-col gap-5">
      <WL2PaymentSummary />
      <div><label className="block text-sm font-semibold text-gray-700 mb-2">Card information</label><div className="rounded-xl border border-gray-200 bg-white p-5"><div className="h-14 rounded-lg bg-gray-100 animate-pulse" /><div className="grid grid-cols-2 gap-3 mt-3"><div className="h-12 rounded-lg bg-gray-100 animate-pulse" /><div className="h-12 rounded-lg bg-gray-100 animate-pulse" /></div></div></div>
      <p className="text-sm text-gray-600">Preparing secure card fields…</p>
    </div>;
  }
  return <>
...
export default function WL2PayPalPaymentForm({ clientId, mode, patientName, patientEmail, patientPhone, onComplete, onError }: WL2PayPalPaymentFormProps) {
  ...
  return <PayPalScriptProvider options={{ clientId, components: "card-fields", intent: "capture", ...(mode === "sandbox" ? { currency: "USD", "buyer-country": "US" } : {}) }}>
    <WL2PayPalProvider orderId={orderId} paymentId={paymentId} onComplete={onComplete} onError={onError} />
